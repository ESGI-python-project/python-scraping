...
def filtre_data(html_content):
    ...
    for one_resultat in data_res:
        list_infos = one_resultat.find('div', {'class': 'fG8Fp', 'class': 'uo4vr'})
        url = one_resultat.find('a').attrs['href']
        if list_infos:  # avoid the search without nots
            if "Note" in str(list_infos):
                noteElement = list_infos.find('span', text=re.compile("Note"))
                if noteElement:
